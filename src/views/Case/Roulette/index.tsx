...
export const Roulette: React.FC<RouletteProps> = ({
  ...
  const { cardWidth, cardMarginRight, containerHorizontalPadding } =
    roulette;

  const maxWidth = getLineMaxWidthByCount(cardCountInBlock, roulette);

  const getIcon = (icon: string, loader?: boolean) => {
  ...
  const items = blocks.map(({ icon, loader }, i) => (
    <CardBuy key={i} width={cardWidth} icon={getIcon(icon, loader)} />
  ));
