...
const contextWithBlocks: Context = {
  ...baseContext,
  blocks: Array.from({ length: baseContext.cardCountInBlock }, () => {
...
describe('generate', () => {
  ...
  it('must work if items === undefined', () => {
    expect(() => generate(cardCountInBlock)).not.toThrowError();
  });

  it('must work if items === null', () => {
    expect(() => generate(cardCountInBlock, null)).not.toThrowError();
  });

  it('generated.length === cardCountInBlock', () => {
    expect(generate(cardCountInBlock)).toHaveLength(cardCountInBlock);
  });
  ...
  it('can override items', () => {
    ...
    expect(generate(cardCountInBlock, items)).toEqual(expected);
  });
});
...
describe('getBlocksWithPrize', () => {
  it('return blocks with prize', () => {
    ...
    const length = context.cardCountInBlock * context.blocksCount - 1;
...
describe('oneLast', () => {
  ...
  it('if blocks.length === cardCountInBlock => return last line', () => {
    expect(oneLast(context)).toHaveLength(context.blocks.length);
  });

  it('if blocks.length !== cardCountInBlock => rerender', () => {
    const innerContext: Context = {
      ...contextWithBlocks,
      blocks: Array.from({ length: 9 }, () => loadingItems[0]),
      cardCountInBlock: 5,
    };
...
describe('waitHandler', () => {
  ...
  it('if isFirst === true => get one random', () => {
    ...
    expect(waitHandler(innerContext) === context.blocks).toBe(false);
  });

  it('if isFirst === false => run one last', () => {
    expect(waitHandler({ ...context, isFirst: false })).toEqual(
      context.blocks,
    );
  });
});
...
describe('startedHandler', () => {
  ...
  it('if newBlocks.length === context.blocks.length => return context.blocks', () => {
    const innerContext: Context = {
      ...context,
      blocks: [...context.blocks, ...getBlocksWithPrize(context)],
    };

    expect(startedHandler(innerContext)).toEqual(innerContext.blocks);
  });
  ...
  it('if newBlocks.length !== context.blocks.length => return new blocks', () => {
    ...
    expect(startedHandler(innerContext)).not.toEqual(context.blocks);
  });
});
...
describe('renderBlocks', () => {
  it('call handler by status', () => {
    ...
    const render = (status: Status) =>
      renderBlocks(status, { ...baseContext }, renderStrategy);
